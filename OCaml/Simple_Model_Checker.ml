...
let rec lexordp_eq _A
  xs ys = match xs, ys with
    x :: xs, y :: ys -> less _A x y || not (less _A y x) && lexordp_eq _A xs ys
    | x :: xs, [] -> false
    | xs, [] -> null xs
    | [], ys -> true;;

let rec less_eq_char
...
let rec lexordp _A
  xs ys = match xs, ys with
    x :: xs, y :: ys -> less _A x y || not (less _A y x) && lexordp _A xs ys
    | xs, [] -> false
    | [], ys -> not (null ys);;
...
type ('a, 'b) hashmap = HashMap of (('a * 'b) list) array * nat;;
     ...
     true, false, false)])
                  ...
                  (shows_quote
                    (shows_prec_list _A zero_nata
                      (take (nat_of_integer (Z.of_int 100)) ts)))));;
...
let rec lx_nat_aux
  acc l =
    bindb (alt (bindb lx_digit
                 (fun x ->
                   lx_nat_aux
                     (plus_nata (times_nat (nat_of_integer (Z.of_int 10)) acc)
...
let rec nats_to_nat
  x xa1 = match x, xa1 with x, [] -> x
    | x, n :: ns ->
        nats_to_nat (plus_nata (times_nat (nat_of_integer (Z.of_int 10)) x) n)
          ns;;
...
let rec as_get s i = (let (a, _) = s in array_get a i);;
...
let rec as_pop s = (let (a, n) = s in as_shrink (a, minus_nat n one_nata));;

let rec as_set s i x = (let (a, b) = s in (array_set a i x, b));;

let rec as_top s = (let (a, n) = s in array_get a (minus_nat n one_nata));;
              ...
              (fun automataa ->
                (let broadcast =
                  ...
                  binda (combine_map of_string broadcast)
                    (fun broadcasta ->
                      (let _ =
                         trace_level (Int_of_integer (Z.of_int 3))
                           (fun _ ->
                             (fun () ->
                               ("Broadcast channels " ^
                                 implode
...
let rec simple_Network_Impl_nat
  broadcast bounds automata m num_states num_actions =
    less_nat zero_nata m &&
      ...
      (all_interval_nat
         (fun i ->
           (let (_, (_, (_, a))) = nth automata i in
             list_all (fun (x, _) -> less_nat x (num_states i)) a))
         zero_nata (size_list automata) &&
        (list_all
           (fun (_, (_, (trans, _))) ->
             list_all
         ...
         (list_all (fun a -> less_nat a num_actions) broadcast &&
           list_all
             (fun (_, (_, (trans, _))) ->
               list_all
                 (fun (_, (_, (_, (a, (_, (_, _)))))) ->
                   pred_act equal_nat (fun aa -> less_nat aa num_actions) a)
                 trans)
             automata) &&
        (list_all
           (fun (_, (_, (trans, _))) ->
            ...
            list_all
              (fun (_, (_, (trans, _))) ->
                list_all
                  (fun a ->
                    (match a
                      with (_, (_, (g, (In aa, (_, (_, _)))))) ->
                        (if membera equal_nat broadcast aa then null g
                          else true)
...
let rec compute_upds_impl
  bounds init =
    map_filter
      (fun comb ->
        (let (g, (a, (r, (l, s)))) =
           fold (fun (q, (_, (g2, (_, (f2, (r2, l2)))))) (g1, (a, (r1, (l, s))))
                  -> (g1 @ g2,
                       (a, (r1 @ r2, (list_update l q l2, mk_updsi s f2)))))
             comb init
           in
          (if check_boundedi bounds s then Some (g, (a, (r, (l, s))))
            else None)));;
    ...
    (let n_ps = size_list automata in
     ...
     let trans_i_map =
       (fun i j ->
         map_filter
           (fun a ->
             (match a with (_, (_, (In _, (_, _)))) -> None
               | (_, (_, (Out _, (_, _)))) -> None
     ...
     let trans_in_map =
       (fun i j ->
         map_filter
           (fun a ->
             (match a
               with (b, (g, (In aa, (ma, l)))) -> Some (b, (g, (aa, (ma, l))))
               | (_, (_, (Out _, (_, _)))) -> None
               | (_, (_, (Sil _, (_, _)))) -> None))
           (trans_mapa i j))
       in
              ...
              (sup_seta equal_literal
                (image
                  (fun (_, (_, (t, _))) ->
                    sup_seta equal_literal
                      (image
                        (fun (_, (_, (_, (a, _)))) -> set_act equal_literal a)
                        (Set t)))
                  (Set automata)))
              (Set broadcast)))
...
let rec renum_upd _A
  renum_vars = (fun (x, upd) -> (renum_vars x, renum_exp _A renum_vars upd));;
...
let rec rename_network _A _B _E _G
  broadcast bounds automata renum_acts renum_vars renum_clocks renum_states =
    (let automataa =
     ...
     let boundsa = map (fun (a, (b, c)) -> (renum_vars a, (b, c))) bounds in
    ...
    sup_set _D
      (sup_seta _D
        (image
          (fun (_, (_, (trans, _))) ->
            sup_seta _D
              (image (fun (_, (_, (_, (a, (_, (_, _)))))) -> set_act _D a)
                (Set trans)))
          (Set automata)))
      (Set broadcast);;
                   ...
                   (intersperse
                     [Chara (false, false, true, true, false, true, false,
                       ...
                       Chara (false, true, false, true, false, false, false,
                               false)]
                     (map (fun (k, v) ->
                            pad (plus_nata n (nat_of_integer (Z.of_int 2)))
                              ([Chara (false, true, false, false, false, true,
false, false)] @
...
false, false)] @
                                shows_json
                                  (plus_nata n (nat_of_integer (Z.of_int 4))) v)
                       xs)) @
                   [Chara (false, true, false, true, false, false, false,
                            false)] @
  ...
  dc json_data =
    (match
      (let s = implode (shows_prec_JSON zero_nata json_data []) in
       let _ =
         trace_level (Int_of_integer (Z.of_int 2)) (fun _ -> (fun () -> s)) in
        binda (parse json s)
          (fun r ->
