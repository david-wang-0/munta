...
(* text \<open>
  ...
  refine it to an efficient imperative version corresponding to the pseudo-code above in the end.
  The main observation underlying the algorithm is that the shortest path from \<open>i\<close> to \<open>j\<close>, only
  using intermediate vertices from the set \<open>{0\<dots>k+1}\<close>, is either: the shortest path from \<open>i\<close> to \<open>j\<close>
...
next
  ...
  next
    ...
    moreover have "x \<noteq> a"
      by (metis (mono_tags) Cons.prems One_nat_def * empty_iff filter.simps(2) impossible_Cons
...
text \<open>
  This lemma gives justifies the use of destructive updates under the absence of negative cycles.
\<close>
lemma fwi_step':
...
text \<open>
  We define the notion of the length of the shortest \<open>simple\<close> path between to vertices,
  using only intermediate vertices from the set \<open>{0..k}\<close>.
\<close>
