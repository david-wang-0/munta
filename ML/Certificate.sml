...
structure Model_Checker : sig
  ...
  datatype 'a state_space =
    Reachable_Set of (('a list * int list) * (int dBMEntry list) list) list |
    Buechi_Set of (('a list * int list) * (int dBMEntry list * nat) list) list
  val e_op_impl :
    nat ->
      (nat, int) acconstraint list ->
        nat list ->
          (nat, int) acconstraint list ->
            (nat, int) acconstraint list ->
              int dBMEntry array -> (unit -> (int dBMEntry array))
  val parse_convert_check :
    mode ->
      nat ->
        bool -> string -> string -> int state_space -> bool -> (unit -> unit)
...
fun as_pop s = let
                 val (a, n) = s;
               in
                 as_shrink (a, minus_nat n one_nata)
               end;
...
fun as_top s = let
                 val (a, n) = s;
               in
                 array_get a (minus_nat n one_nata)
               end;
...
fun fwi_impl_int n a k =
  let
    ...
    val n_plus_1 = IntInf.+ (na, (1 : IntInf.int));
  in
    imp_for_int_innera (0 : IntInf.int) n_plus_1
      (fn i =>
        imp_for_int_innera (0 : IntInf.int) n_plus_1
          ...
          (sup_set equal_literal
            (sup_seta equal_literal
              (image
                (fn (_, (_, (t, _))) =>
                  sup_seta equal_literal
                    (image (fn (_, (_, (_, (a, _)))) => set_act equal_literal a)
                      (Set t)))
                (Set automata)))
            (Set broadcast)))
      ...
      assert
        (list_all
          (fn (_, (_, (trans, _))) =>
            list_all
              (fn (_, (_, (_, (a, (_, (_, _)))))) =>
                pred_act equal_nat (fn aa => less_nat aa num_actions) a)
              trans)
          automata)
  ...
  sup_set D_
    (sup_seta D_
      (image
        (fn (_, (_, (trans, _))) =>
          sup_seta D_
            (image (fn (_, (_, (_, (a, (_, (_, _)))))) => set_act D_ a)
              (Set trans)))
        (Set automata)))
    (Set broadcast);
...
fun print_sep x =
  (fn () =>
    writeln (implode
              (replicate (nat_of_integer (100 : IntInf.int))
                (Chara (true, false, true, true, false, true, false, false)))))
    x;
      ...
      Le zero_inta ::
        insert_every_nth m INF (drop m dbm) @
          (if urge then Le zero_inta else INF) ::
            replicate (minus_nat m one_nata) INF @ [Le zero_inta]);
            ...
            (fn renaminga =>
              binda (convert_renaming ids_to_names process_names_to_index
                ...
                (fn (var_renaming,
                   ...
                   =>
                  binda (make_renaming (equal_nat, show_nat) broadcast automata
                          bounds)
                    (fn (m, (num_states,
                              (num_actions,
                                (renum_acts,
        ...
        in
          (fn f_ => fn () => f_
            ((certify_unreachable_impl_inner
               ...
               succsi l_0i s_0i (split_ka num_split) l_list m_table)
            ()) ())
            (fn a => (fn () => a))
        end)
  end;
...
(fn x_e => (fn () => (if x_e then sigmaa else op_list_prepend x_c sigmaa)))))
  ...
  in
    certify_unreachable_impl2
      ...
      s_0i (split_ka num_split) l_list a
  end;
...
fun certify_unreachable_impl_pure get_succs li lei li_split pi fi mi l_0i s_0i =
  let
    val check_all_pre_impl =
      time_it "Time for state set preconditions check"
        (fn _ =>
...
fun equal_mode Buechi Debug = false
  ...
  (case binda (parse json s) convert
    of Result
      ...
      => (case rename_state_space show_literal dc ids_to_names
           ...
           of Result (NONE, (_, (_, (_, _)))) => (fn () => ())
           | Result (SOME r, (_, (_, (renamings, k)))) =>
             let
               val t = Time.now ();
             in
               (fn f_ => fn () => f_ (r ()) ())
...
show_lit show_nat (size_list state_space)))
 ...
 writeln ("Size of passed list: " ^
           show_str show_nat
             (sum_list monoid_add_nat (map (size_list o snd) ra)));
                                     in
                                       (fn f_ => fn () => f_
 ((print_line_impl
             ...
             (fn check =>
               ...
               in
                 (fn f_ => fn () => f_
                   ...
                   (fn _ =>
                     (case check
                       of Renaming_Failed => print_line_impl "Renaming failed"
                       | Preconds_Unsat =>
                         print_line_impl "Preconditions were not met"
                       | Sat => print_line_impl "Certificate was accepted"
                       | Unsat => print_line_impl "Certificate was rejected"))
               end)
         end)
   end)
                                     end)
  ...
  (case binda (parse json s) convert
    of Result
      ...
      => (case rename_state_space show_literal dc ids_to_names
                 (broadcast, (automata, bounds)) l_0 s_0 formula
           of Result (NONE, (_, (_, (_, _)))) => (fn () => ())
           | Result (SOME r, (show_clk, (show_st, (_, _)))) =>
             (fn f_ => fn () => f_ (r ()) ())
               (fn ra =>
                 let
                   val _ =
                     writeln ("Number of discrete states: " ^
                               show_str show_nat (size_list ra));
                   val _ =
                     writeln ("Size of passed list: " ^
                               show_str show_nat
                                 (sum_list monoid_add_nat
                                   (map (size_list o snd) ra)));
                   val n =
                     size_list
                       (list_of_set equal_literal
                         ...
                         val show_dbm =
                           (fn m =>
                             (fn f_ => fn () => f_
                               ...
                               (fn sa =>
                                 (fn () =>
                                   ([Chara (false, false, true, true, true,
     true, false, false)] @
                                     sa @ [Chara
     (false, true, true, true, true, true, false, false)]))));
                       in
                           ...
                           ((imp_map
                              (fn (sa, xs) =>
                                let
                                  val sb = show_st sa;
                                in
                                  (fn f_ => fn () => f_ ((imp_map show_dbm xs)
                                    ()) ())
                                    (fn xsa =>
                                      let
val _ =
  writeln (implode
            (sb @ [Chara (false, true, false, true, true, true, false, false),
                    Chara (false, false, false, false, false, true, false,
                            false)] @
